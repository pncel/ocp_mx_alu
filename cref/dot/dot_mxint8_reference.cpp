...
FP32_ieee754 to_FP32(int32_t MXINT8_sum_result, int32_t shared_scale){
    ...
    if(shared_scale - 6 + MXINT8_sum_bitcnt -1 > 0){ 
        ...
        if(shared_scale - 6 + MXINT8_sum_bitcnt -1 < 255){ 
            ...
            FP32_sum_result.exponent = shared_scale - 6 + MXINT8_sum_bitcnt -1; // VERIFY WORKS BOTH DIR. (think so 2/11)
        }
        //overflow -> clamp
        else {
    ...
    //*************************subnormal case*************************
        ...
        //mantissa; shift so that leading 0 "falls off the cliff" (implicit leading 0 for subnormal not encoded)
