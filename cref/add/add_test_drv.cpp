// comment: 
// g++ -o test add_test_drv.cpp
// ./test > add_testcases.txt
...
void publish_testcase_2vec(MXINT8_vector testcase_A, MXINT8_vector testcase_B) {
    ...
    cout << "scale_B = " << testcase_B.scale.to_ulong() << endl << endl;
    for (int i = 0; i < 32; i++) {
        cout << "expected_result[" << i << "] = " << endl;
    }
    cout << "scale_Y = " << endl;
...
int main(){
    ...
    cout << "\nA add B all_large_positive_elements"<<endl;
    for(int i = 0; i<3; i++){
        cout << "\nCase = " << case_cnt << endl;
