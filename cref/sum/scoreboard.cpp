...
void processFile_and_answer(const string& fileName) {
    ...
    while (getline(inputFile, line)) {
        ...
        if (line.find("element[") != string::npos) {
            ...
            sscanf(line.c_str(), "element[%d] = %d", &elementIndex, &decimalValue);
            scalar_in.elements[elementIndex] = decimalValue;
            continue;
        ...
        if (line.find("expected") != string::npos) {
            ...
            //sum the scalar
            actual_result = sum_reference(scalar_in);
...
int main(){
    ...
    string fileName = "testcase_ref.txt"; 
